...
void block_convert_float(std::ofstream& writer, std::ifstream& reader,
...
void block_convert_int8(std::ofstream& writer, std::ifstream& reader,
                        int8_t* read_buf, _u64 npts, _u64 ndims) {
  reader.read((char*) read_buf, npts * ndims * sizeof(float));

  for (_u64 i = 0; i < npts; i++) {
    for (_u64 d = 0; d < ndims; d++) {
      writer << (int) read_buf[d + i * ndims];
      if (d < ndims - 1)
        writer << "\t";
      else
        writer << "\n";
...
void block_convert_uint8(std::ofstream& writer, std::ifstream& reader,
                         uint8_t* read_buf, _u64 npts, _u64 ndims) {
  reader.read((char*) read_buf, npts * ndims * sizeof(float));

  for (_u64 i = 0; i < npts; i++) {
    for (_u64 d = 0; d < ndims; d++) {
      writer << (unsigned int) read_buf[d + i * ndims];
      if (d < ndims - 1)
        writer << "\t";
      else
        writer << "\n";
...
int main(int argc, char** argv) {
  ...
  for (_u64 i = 0; i < nblks; i++) {
    ...
    if (type_string == std::string("float"))
      block_convert_float(writer, reader, (float*) read_buf, cblk_size, ndims);
    else if (type_string == std::string("int8"))
      block_convert_int8(writer, reader, (int8_t*) read_buf, cblk_size, ndims);
    else if (type_string == std::string("uint8"))
      block_convert_uint8(writer, reader, (uint8_t*) read_buf, cblk_size,
                          ndims);
